...
const LadiesBag = () => {
  ...
  const products = staticProducts.filter(p => p.category === "Ladies Bag").slice(0, 10);
  ...
  return (
    <section className="container mx-auto px-3 sm:px-4 py-6 sm:py-10">
      <div className="mb-4 sm:mb-6 flex items-center justify-between">
        <h2 className="text-sm sm:text-lg font-bold text-foreground">Ladies Bag</h2>
        <Link to="/category/ladies-bag" className="flex items-center gap-1 rounded-full border border-border px-3 sm:px-4 py-1 sm:py-1.5 text-[10px] sm:text-xs font-medium text-foreground/70 hover:border-primary/40 hover:text-primary transition-all">
